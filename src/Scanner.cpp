...
void Scanner::init(std::string in) {
  unsigned int idx = 0;
  ...
  while (idx < in.length()) {
    ...
    case '\\': {
      ...
      case 'S':
        ...
        token.character = c;
        break;
      // Treat \A the same as ^ (only differ in multi-line which is not
      // supported)
      case 'A':
        token.type = CARET;
        break;
      // Treat \Z the same as $ (only differ in multi-line which is not
      // supported)
      case 'Z':
        token.type = DOLLAR;
        break;
      // \b is backspace in a character set (unsupported) and word boundary
      // otherwise
      case 'b':
        if (in_set) {
      ...
      // Escaped characters are unsupported for test generation but supported
      // for check mode
      ...
      case 'a':
        if (Util::get()->is_check_mode()) {
          ...
          throw EgretException(
              "ERROR (unsupported): contains unsupported character \\a");
        }
        break;
      case 'f':
        if (Util::get()->is_check_mode()) {
          ...
          throw EgretException(
              "ERROR (unsupported): contains unsupported character \\f");
        }
        break;
      case 'n':
        if (Util::get()->is_check_mode()) {
      ...
      case 'r':
        if (Util::get()->is_check_mode()) {
          ...
          throw EgretException(
              "ERROR (unsupported): contains unsupported character \\r");
        }
        break;
      case 't':
        if (Util::get()->is_check_mode()) {
      ...
      case 'v':
        if (Util::get()->is_check_mode()) {
          ...
          throw EgretException(
              "ERROR (unsupported): contains unsupported character \\v");
        }
        break;
      case 'p':
        throw EgretException(
            "ERROR (unsupported): contains unsupported character \\p");
      case '\\':
        token.type = CHARACTER;
        token.character = '\\';
        break;
    ...
    case '{':
      ...
      // otherwise --> process a possible repeat clause
        ...
        if (token.type != CHARACTER && (idx + 1) < in.length() &&
            in[idx + 1] == '?') {
          idx++;
        }
      }
...
char Scanner::get_next_char(std::string in, unsigned int &idx) {
  idx++;
  if (idx >= in.length()) {
...
Token Scanner::process_octal(std::string in, unsigned int &idx,
                             char first_digit) {
  ...
  if (only_one_digit) {
    if (first_digit == '0') {
      if (Util::get()->is_check_mode()) {
        ...
        throw EgretException(
            "ERROR (unsupported): contains unsupported character \\0");
      }
    } else {
  ...
  if (octal_found) {
    ...
    if (octal_value > 126 ||
        (octal_value < 32 && !Util::get()->is_check_mode())) {
      std::stringstream s;
      s << "ERROR (unsupported): contains unsupported octal value "
        << octal_value;
...
Token Scanner::process_extension(std::string in, unsigned int &idx) {
  ...
  case 'P': {
    ...
    if (c == '=') {
      ...
      throw EgretException("ERROR (parse error): Improperly specified named "
                           "group - expected < after (?P");
    } else {
      std::stringstream s;
      while (c != '>') {
        c = get_next_char(in, idx);
  ...
  // TODO: These are not test - especially comments which should be ignored up
  // until the )
  case '#':
  case '=':
  case '!':
...
Token Scanner::process_repeat(std::string in, unsigned int &idx) {
  ...
  //
  // The values n and m must be integers consisting of one or more digits.  Any
  // deviation from the above, including the addition of spaces, will result in
  // a literal match. For example, the only string to match the regular
  // expression "a{3, 4}" is "a{3, 4}".
  //
  // The only error that is possible is when the repeat quantifier is of the
  // form {n,m} and n > m.
  //
  // This program will abort on pointless quantifiers {0}, {,0}, and {0,0}.

  // Save the current index and a default return struct that matches the "{"
  // only.  If there is a formatting error, idx will be restored to the current
  // index and the "{" character token is returned.
  //
  int current_idx = idx;
  Token token;
  token.loc.first = idx;
  ...
  // Stopping chatacter is something else --> literal match (ill-formed
  // quantifier)
  else {
    idx = current_idx;
    return token;
  ...
  if (c == '}') {
    ...
    // Check for no bounds, at least one must be present.  If not --> literal
    // match
    ...
    if (token.repeat_lower == -1)
      token.repeat_lower = 0;
    ...
    if (token.repeat_lower > token.repeat_upper) {
      std::stringstream s;
      s << "ERROR (parse error): Invalid repeat quantifier: lower bound "
        << token.repeat_lower << " is greater than upper bound "
        << token.repeat_upper << std::endl;
      throw EgretException(s.str());
    }
    ...
    if (token.repeat_upper == 0) {
      throw EgretException(
          "ERROR (pointless repeat): pointless repeat quantifier {0,0}");
    }
  ...
  // Stopping character is somethine else --> literal match (ill-formed
  // quantifier)
  else {
    idx = current_idx;
    return token;
...
std::string Scanner::get_type_str() { return token_type_to_str(get_type()); }

Location Scanner::get_loc() {
  if (index < tokens.size()) {
...
std::string Scanner::get_group_name() {
  TokenType type = get_type();
...
void Scanner::advance() { index++; }

bool Scanner::is_concat() {
  ...
  bool valid_prev_type =
      prev_type == STAR || prev_type == PLUS || prev_type == QUESTION ||
      prev_type == REPEAT || prev_type == RIGHT_PAREN ||
      prev_type == CHARACTER || prev_type == CARET || prev_type == DOLLAR ||
      prev_type == WORD_BOUNDARY || prev_type == CHAR_CLASS ||
      prev_type == RIGHT_BRACKET || prev_type == BACKREFERENCE;

  bool invalid_next_type = next_type == ALTERNATION || next_type == STAR ||
                           next_type == PLUS || next_type == QUESTION ||
                           next_type == REPEAT || next_type == RIGHT_PAREN ||
                           next_type == RIGHT_BRACKET;

  return valid_prev_type && !invalid_next_type;
}

bool Scanner::is_char_range() {
  if (index > tokens.size() - 3)
    return false;
  if (tokens[index + 1].type != HYPHEN)
    return false;

  if (tokens[index].type == CHARACTER && tokens[index + 2].type == CHARACTER) {
    return true;
  }
  return false;
}
...
std::string Scanner::token_type_to_str(TokenType type) {
  switch (type) {
  case ALTERNATION:
    return "ALTERNATION";
  case STAR:
  ...
  case ERR:
    return "<ERROR> (or end of regex)";
  default:
    throw EgretException("ERROR (internal): unexpected token type");
  }
}

void Scanner::add_stats(Stats &stats) {
